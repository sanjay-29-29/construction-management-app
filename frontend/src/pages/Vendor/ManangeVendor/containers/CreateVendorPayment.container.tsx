...
export const CreateVendorPayment = ({
  dialogOpen,
  setDialogOpen,
}: {
  dialogOpen: boolean;
  ...
  const mutation = useMutation({
    mutationFn: async (data: RatePaymentFormValues) => {
      await client.post(`vendors/${vendorId}/payments/`, data);
    },
    onSuccess: () => {
      setDialogOpen(false);
      toast.success('The payment entry was created successfully.');
    ...
    onError: (error) => {
      if (isAxiosError(error)) {
        toast.error('Error occurred while creating payment entry.');
        return;
      }
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Add payment entry here. Click save when you're done.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
